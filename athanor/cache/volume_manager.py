...
Volume naming convention:
  per-job  → athanor-qa-vol-<job_id>
  per-repo → athanor-qa-vol-<sanitized-repo>
  per-org  → athanor-qa-vol-<sanitized-org>   (Phase 2 raises NotImplementedError)
...
def volume_name_for(scope: VolumeScope, scope_key: str) -> str:
    if scope is VolumeScope.PER_JOB:
        return f"athanor-qa-vol-{_sanitize(scope_key)}"
