"""Pydantic schema for `.athanor/qa.yaml` v2 — multi-app monorepo contract.

v2 replaces v1 (single-app, no workspace concept). The migrator at
athanor/cli/migrate_qa_config.py converts existing v1 files. v1 reading
remains via athanor.workflows.qa.qa_yaml.parse_qa_yaml — used ONLY by the
migrator now.
"""
...
class AppEntry(BaseModel):
    ...
    Heavy overrides go in apps/<name>/.athanor/app.yaml (parsed separately).
    """
...
class QAYamlConfigV2(BaseModel):
    """Top-level .athanor/qa.yaml v2 contract."""
...
class AppLocalConfig(BaseModel):
    """Schema for apps/<name>/.athanor/app.yaml — heavy overrides only."""
