...
from embry0.api.auth import verify_webhook_signature
...
async def github_webhook(
    ...
    # inbound dispatcher (embry0.notifications.inbound_dispatch) so the same
    ...
    if event_type == "pull_request" and action in ("opened", "closed"):
        ...
        if repo_name and pr_url:
            ...
            # Match PR to job via branch name pattern: embry0/{id}-{slug}
            if branch.startswith("embry0/"):
