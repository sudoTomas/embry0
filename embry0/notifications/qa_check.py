"""Orchestrator-side embry0/qa GitHub Check Run writer.

A single check named `embry0/qa` gates each PR. The check is created or
...
from embry0.workflows.qa.subtask_result_schema import FAILED_STATUSES, SubTaskResult, SubTaskStatus

logger = structlog.get_logger(__name__)

_GITHUB_API = "https://api.github.com"
CHECK_NAME = "embry0/qa"
...
def build_check_payload(
    *,
    ...
    Maps embry0's outcome semantics onto GitHub's check status + conclusion:
    ...
    if overall_status == "infra_error":
        return QACheckPayload(
            ...
            summary=failure_summary or "embry0 encountered an infra error.",
        )
...
async def _find_existing_check_run_id(
    ...
    *,
    ...
    """Look up an existing embry0/qa check run for this head_sha. Idempotent."""
...
async def write_aggregate_check(
    *,
    ...
    """Create or update the embry0/qa GitHub Check Run for this head_sha.

    Idempotent: if a check with name=embry0/qa already exists for this commit,
