...
Observed in production on a large monorepo (2026-07-08): the host
exposes 47 CPUs, the sandbox has ``--cpus=4 --pids-limit=256``, and
Next.js tries to spawn 47 page-data-collection workers → EAGAIN →
build exits 1.
...
def compute_worker_env(cpus: str | int | float) -> dict[str, str]:
    ...
    so user-provided overrides (set in the embry0 env-vars UI) take
    precedence.
