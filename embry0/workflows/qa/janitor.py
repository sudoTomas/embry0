...
scheduled by the existing embry0 periodic-task runner (the wiring of which
...
class _DockerLike(Protocol):
    async def list_containers_with_label(self, label_prefix: str = "embry0.qa_job_id") -> list[dict[str, Any]]: ...
...
async def reap_orphan_sandboxes(
    ...
    """Find embry0-labelled containers whose parent run is gone; reap them.
    ...
    containers = await docker.list_containers_with_label("embry0.qa_job_id")

    for c in containers:
        sub_job_id = (c.get("labels") or {}).get("embry0.qa_job_id")
