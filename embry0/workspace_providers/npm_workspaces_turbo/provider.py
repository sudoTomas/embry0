...
from embry0.workspace_providers.npm_workspaces_turbo._discover import (
    discover_workspaces,
)
...
class NpmWorkspacesTurboProvider:
    ...
    def validate(self, apps_declared_in_qa_config: list[str]) -> list[str]:
        ...
        for orphan in sorted(workspace_app_names - declared):
            messages.append(
                f"warning: app {orphan!r} present in workspace but missing from qa.yaml apps: — embry0 will not QA it"
            )
