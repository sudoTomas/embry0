...
def _summarize_tool_input(tool_name: str, tool_input: dict) -> str:
    ...
    if tool_name in ("Read", "Glob", "Grep"):
        return tool_input.get("file_path", "") or tool_input.get("path", "") or tool_input.get(
            "pattern", ""
        )
    ...
    async def run(
        ...
        def _is_assistant(msg: Any) -> bool:
            if isinstance(msg, AssistantMessage):
                return True
            return hasattr(msg, "content") and hasattr(msg, "model") and not hasattr(
                msg, "total_cost_usd"
            )
        ...
        def _is_result(msg: Any) -> bool:
            if isinstance(msg, ResultMessage):
                return True
            return hasattr(msg, "total_cost_usd") or (
                hasattr(msg, "result") and hasattr(msg, "num_turns")
            )

        def _is_text_block(blk: Any) -> bool:
            if isinstance(blk, TextBlock):
                return True
            return hasattr(blk, "text") and not hasattr(blk, "name") and not hasattr(
                blk, "input"
            )
        ...
        (claude_dir / "settings.json").write_text(
            json.dumps(render_settings_json(invocation.safety_policy), indent=2)
        )
