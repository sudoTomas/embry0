...
async def test_reap_orphan_sandboxes_removes_containers_for_inactive_runs():
    ...
    docker.list_containers_with_label = AsyncMock(return_value=[
        {"id": "c1", "labels": {"athanor.qa_parent_run_id": "run-A"}},
        {"id": "c2", "labels": {"athanor.qa_parent_run_id": "run-B"}},
        {"id": "c3", "labels": {"athanor.qa_parent_run_id": "run-A"}},
    ])
...
async def test_reap_continues_when_one_container_kill_fails():
    docker = AsyncMock()
    docker.list_containers_with_label = AsyncMock(return_value=[
        {"id": "c1", "labels": {"athanor.qa_parent_run_id": "dead"}},
        {"id": "c2", "labels": {"athanor.qa_parent_run_id": "dead"}},
    ])
