...
from athanor.workflows.qa.boot import BootResult, run_boot_phase
...
async def test_multiple_ready_checks_all_must_pass():
    ...
    result = await run_boot_phase(
        qa_yaml=qa_yaml, container_id="C", docker=docker, http=http, sleep_seconds=0,
    )
    assert result.outcome == "passed"
...
async def test_records_per_check_diagnostics_on_timeout():
    ...
    qa_yaml = _make_qa_yaml(boot_timeout=0.3, ready_checks=[
        {"http": "http://gateway:8080/health", "expect_status": 200},
        {"http": "http://frontend:3000/", "expect_status": 200},
    ])
    result = await run_boot_phase(
        qa_yaml=qa_yaml, container_id="C", docker=docker, http=http, sleep_seconds=0.05,
    )
    assert result.outcome == "timeout"
