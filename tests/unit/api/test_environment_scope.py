...
def test_input_rejects_qa_artifact_prefix():
    with pytest.raises(ValidationError, match=r"reserved prefix 'QA_ARTIFACT_'"):
        EnvVarInput(key="QA_ARTIFACT_BUCKET", value="x", scope="qa")


def test_input_rejects_docker_prefix():
    with pytest.raises(ValidationError, match=r"reserved prefix 'DOCKER_'"):
        EnvVarInput(key="DOCKER_BUILDKIT", value="x", scope="app")
