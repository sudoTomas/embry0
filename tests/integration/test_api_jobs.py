...
async def test_list_jobs_with_filter(app: AsyncClient):
    await app.post("/api/v1/jobs", json={"repo": "a/b", "task": "Task 1"}, headers=CSRF)
    await app.post("/api/v1/jobs", json={"repo": "a/b", "task": "Task 2"}, headers=CSRF)
    await app.post("/api/v1/jobs", json={"repo": "c/d", "task": "Task 3"}, headers=CSRF)
...
async def test_cancel_job(app: AsyncClient):
    resp = await app.post(
        "/api/v1/jobs",
        json={"repo": "owner/repo", "task": "Cancel me"},
        headers=CSRF,
    )
    job_id = resp.json()["job_id"]

    resp = await app.post(f"/api/v1/jobs/{job_id}/cancel", headers=CSRF)
    assert resp.status_code == 200
