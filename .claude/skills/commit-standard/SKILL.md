...
description: Athanor's git commit standard. Use this skill EVERY time you write a commit message — distilled from Chris Beams 7-rules, Tim Pope 50/72, and Conventional Commits, anchored to Athanor's existing scope vocabulary.
---

# Athanor Commit Standard

Adapted from the Ormus Commit Standard (OCS v1.0). The shape is **Conventional Commits** with strict 50/72 + Athanor's house scopes.
...
- Reference issues / specs by path: `docs/superpowers/specs/YYYY-MM-DD-<slug>-design.md`
...
## Athanor scope vocabulary

The scope is the closest meaningful unit. Use these existing scopes from Athanor's git history (per `git log --oneline`):

| Scope | Use when touching |
|---|---|
| `qa` | `athanor/workflows/qa/`, qa-related agent seed, qa pipeline graph |
...
| `stats` | `athanor/api/v1/stats.py` + stats response shape |
| `agents` | `athanor/agents/` + `athanor/storage/repositories/agent_definitions.py` |
| `triage` | The triage workflow node |
| `sandbox` | `athanor/sandbox/` + sandbox image / Dockerfile.sandbox |
| `proxy` | `athanor/execution/proxy/` |
| `storage` | `athanor/storage/` (migrations, repositories) |
| `api` | New `athanor/api/v1/<file>.py` endpoint |
...
## Type taxonomy (Athanor practice)
...
## Anti-goals (matches Athanor's existing anti-goal style)
...
## Quick reference: subject patterns from Athanor's existing history
